...
namespace Command.ChatApp
{
    public class CommandClient
    {
        ...
        public CommandClient(IPEndPoint server, string netName)
        {
            ...
            System.Net.NetworkInformation.NetworkChange.NetworkAvailabilityChanged += new System
                .Net
                .NetworkInformation
        ...
        private void StartReceive(object sender, DoWorkEventArgs e)
        {
            while (this.clientSocket.Connected)
            {
                ...
                // Read the commands sender IP size
                buffer = new byte[4];
                ...
                // Read the commands sender name size
                buffer = new byte[4];
                readBytes = this.networkStream.Read(buffer, 0, 4);
                if (readBytes == 0)
                {
                ...
                //Read the commands MetaData size.
                string cmdMetaData = "";
                buffer = new byte[4];
                readBytes = this.networkStream.Read(buffer, 0, 4);
                if (readBytes == 0)
        ...
        private void bwSender_RunWorkerComplited(object sender, RunWorkerCompletedEventArgs e)
        {
        ...
        private void bwConnector_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                this.clientSocket.Connect(this.serverIP);
                e.Result = true;
